package ru.trifonov.clubfirst.views
...
class SwipeView @JvmOverloads constructor(
    ...
    var onCardBind: ((View, MainFragment.SomeUsefulData) -> Unit)? = null

    var onAllCardSwiped: (() -> Unit)? = null
    var onCardSwiped: ((View, Any?, SwipeDirection) -> Unit)? = null
    var onCardSwipingPercentChanged: ((View, Float, SwipeDirection?) -> Unit)? = null
            ...
            when (event.action) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    if (distX > 0) {
                        ...
                        onCardSwipingPercentChanged?.invoke(view, swipePercent, SwipeDirection.RIGHT)
                    } else {
                        view.rotation = max(-MAX_ANGLE, distX / 35f)
                        onCardSwipingPercentChanged?.invoke(view, swipePercent, SwipeDirection.LEFT)
                    }
                ...
                MotionEvent.ACTION_CANCEL -> {
                    when {
                        ...
                        lastX <= -MIN_SWIPE_BARRIER_X -> {
                            ...
                            onCardSwipingPercentChanged?.invoke(view, 0f, null)
                            view.animate()
                                .x(0f)
                                .y(0f)
                                .rotation(0f)
...
data class SwipeCardItem(
    ...
    val data: MainFragment.SomeUsefulData
)
